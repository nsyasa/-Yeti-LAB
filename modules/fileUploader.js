...
const FileUploader = {
    ...
    async uploadFile(file, studentId, assignmentId, onProgress = null) {
        ...
        try {
            ...
            const result = await this._uploadWithProgress(supabase, this.config.bucket, path, file, (progress) => {
    ...
    async uploadMultiple(files, studentId, assignmentId, onTotalProgress = null) {
        ...
        for (const file of fileArray) {
            const result = await this.uploadFile(file, studentId, assignmentId, (progress) => {
                // Calculate total progress
                const totalProgress = Math.round((completedCount * 100 + progress) / fileArray.length);
    ...
    async getSignedUrl(path, expiresIn = 3600) {
        try {
            ...
            const { data, error } = await supabase.storage.from(this.config.bucket).createSignedUrl(path, expiresIn);
    ...
    async downloadFile(path) {
        try {
            ...
            const { data, error } = await supabase.storage.from(this.config.bucket).download(path);
    ...
    async listFiles(folder) {
        try {
            ...
            const { data, error } = await supabase.storage.from(this.config.bucket).list(folder, {
                limit: 100,
