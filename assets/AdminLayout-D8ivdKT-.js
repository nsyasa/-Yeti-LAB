const a={renderTabNav(){return`
        ...
        `},updateUserInfo(){console.log("[AdminLayout] User info managed by main navbar")},updateActiveTab(t){document.querySelectorAll(".admin-tab-btn").forEach(e=>{e.classList.remove("bg-theme","text-white"),e.classList.add("text-gray-600","dark:text-gray-300","hover:bg-gray-200","dark:hover:bg-gray-600"),e.dataset&&e.dataset.section===t&&(e.classList.add("bg-theme","text-white"),e.classList.remove("text-gray-600","dark:text-gray-300","hover:bg-gray-200","dark:hover:bg-gray-600"))})},async handleLogout(){if(confirm("Çıkış yapmak istediğinizden emin misiniz?"))try{window.SupabaseClient&&await SupabaseClient.signOut(),AdminView.unmount(),Router.navigate("/")}catch(t){console.error("[AdminLayout] Logout error:",t)}}};window.AdminLayout=a;
