const n={render(){return`
        ...
        `},renderComponentCard(e){return`
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-4 hover:border-theme/50 hover:shadow-md transition cursor-pointer group"
                 onclick="ComponentManager.edit('${e.id}')">
                ...
                <h3 class="font-bold text-gray-700 dark:text-gray-200 mb-1">${this.escapeHtml(e.name||"İsimsiz")}</h3>
                <p class="text-xs text-gray-400 dark:text-gray-400 line-clamp-2">${this.escapeHtml(e.description||"")}</p>
                
                <!-- Actions -->
                <div class="flex justify-end gap-1 mt-3 opacity-0 group-hover:opacity-100 transition">
                    <button onclick="event.stopPropagation(); ComponentManager.edit('${e.id}')" 
                            class="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 rounded text-sm" title="Düzenle">
                        ✏️
                    </button>
                    <button onclick="event.stopPropagation(); ComponentManager.deleteById('${e.id}')" 
                            class="p-1.5 hover:bg-red-100 dark:hover:bg-red-900/30 text-red-500 dark:text-red-400 rounded text-sm" title="Sil">
                        🗑️
                    </button>
                </div>
            </div>
        `},renderEditModal(){return`
        ...
        `},filterList(e){const t=document.querySelectorAll("#component-list > div"),r=e.toLowerCase();t.forEach(a=>{const o=a.querySelector("h3")?.textContent?.toLowerCase()||"",d=a.querySelector("p")?.textContent?.toLowerCase()||"";o.includes(r)||d.includes(r)?a.classList.remove("hidden"):a.classList.add("hidden")})},filterByCategory(e){window.ComponentManager&&ComponentManager.filterByCategory&&ComponentManager.filterByCategory(e)},getCategoryLabel(e){return{basic:"Temel",sensor:"Sensör",actuator:"Aktüatör",display:"Ekran",communication:"İletişim",power:"Güç",other:"Diğer"}[e]||e||"Diğer"},escapeHtml(e){const t=document.createElement("div");return t.textContent=e,t.innerHTML}};window.ComponentsSection=n;
