const c={courses:[],init(){this.refreshList()},refreshList(){if(this.courses=[],typeof admin<"u"&&admin.allCourseData&&Object.keys(admin.allCourseData).length>0)this.courses=Object.entries(admin.allCourseData).map(([e,t])=>({key:e,title:t.title||e,description:t.description||"",icon:t.icon||"📦",position:t._position!==void 0?t._position:999,_supabaseId:t._supabaseId}));else if(typeof CourseLoader<"u"){const e=CourseLoader.getManifest();this.courses=Object.entries(e).map(([t,r])=>({key:t,title:r.title||t,description:r.description||"",icon:r.icon||"📦",position:r.position!==void 0?r.position:999}))}this.courses.sort((e,t)=>{const r=e.position!==void 0?e.position:999,s=t.position!==void 0?t.position:999;return r-s})},renderSelectorGrid(){const e=document.getElementById("course-selector-grid");if(!e)return;if(this.courses.length===0){e.innerHTML='<div class="col-span-full text-center text-gray-400 py-4">Kayıtlı kurs yok.</div>';return}const t=admin.currentCourseKey;e.innerHTML=this.courses.map((r,s)=>{const o=r.key===t,a=s===0,n=s===this.courses.length-1,i=Utils.escapeHtml(r.key),d=Utils.escapeHtml(r.title),l=Utils.escapeHtml(r.icon||"📦");return`
            <div class="relative group">
                <button type="button" onclick="CourseManager.selectCourse(${JSON.stringify(String(r.key))})"
                    class="w-full p-3 rounded-lg border-2 transition text-left ${o?"border-theme bg-theme/10 dark:bg-theme/20 shadow-md":"border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-gray-300 dark:hover:border-gray-600 hover:shadow"}">
                    <div class="flex items-center gap-2">
                        <span class="text-2xl">${l}</span>
                        <div class="flex-1 min-w-0">
                            <div class="font-bold text-sm text-gray-800 dark:text-gray-100 truncate">${d}</div>
                            <div class="text-xs text-gray-400 dark:text-gray-500 font-mono truncate">${i}</div>
                        </div>
                    </div>
                </button>
                ...
                <div class="absolute -right-1 top-1/2 -translate-y-1/2 flex flex-col gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button type="button" onclick="event.stopPropagation(); CourseManager.moveUp(${s})" 
                        class="w-5 h-5 rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-xs ${a?"invisible":""}"
                        title="Yukarı">↑</button>
                    <button type="button" onclick="event.stopPropagation(); CourseManager.moveDown(${s})" 
                        class="w-5 h-5 rounded bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-xs ${n?"invisible":""}"
                        title="Aşağı">↓</button>
                </div>
            </div>`}).join("")},selectCourse(e){typeof admin<"u"&&admin.changeCourse&&(admin.changeCourse(e),this.renderSelectorGrid())},showInlineAddForm(){const e=document.getElementById("inline-add-course-form");e&&(e.classList.remove("hidden"),document.getElementById("inline-course-icon").value="📚",document.getElementById("inline-course-title").value="",document.getElementById("inline-course-key").value="",document.getElementById("inline-course-desc").value="",document.getElementById("inline-course-title").focus())},hideInlineAddForm(){const e=document.getElementById("inline-add-course-form");e&&e.classList.add("hidden")},async createFromInline(){const e=document.getElementById("inline-course-icon").value||"📚",t=document.getElementById("inline-course-title").value;let r=document.getElementById("inline-course-key").value;const s=document.getElementById("inline-course-desc").value;if(r&&(r=r.toLowerCase().trim().replace(/_/g,"-").replace(/\s+/g,"-").replace(/[^a-z0-9-]/g,""),document.getElementById("inline-course-key").value=r),!t||!r){alert("Lütfen başlık ve anahtar alanlarını doldurun.");return}if(!/^[a-z0-9-]+$/.test(r)){alert("Anahtar sadece küçük harf, rakam ve tire içerebilir.");return}if(this.courses.find(o=>o.key===r)){alert("Bu anahtara sahip bir kurs zaten var.");return}try{typeof admin<"u"&&admin.showLoading&&admin.showLoading("Kurs oluşturuluyor...");let o=null;if(typeof SupabaseClient<"u"&&SupabaseClient.client){const{data:a,error:n}=await SupabaseClient.client.from("courses").insert({slug:r,title:t,description:s,meta:{icon:e},is_published:!0,position:this.courses.length}).select("id").single();if(n)throw n;o=a.id,await SupabaseClient.client.from("phases").insert({course_id:o,name:"Başlangıç",description:"İlk adımlar",position:0,meta:{color:"blue",icon:"🚀"}})}typeof admin<"u"&&(admin.allCourseData[r]={title:t,description:s,icon:e,_supabaseId:o,_phaseIds:{},_projectIds:{},data:{phases:[{color:"blue",title:"Başlangıç",description:"İlk adımlar",icon:"🚀"}],projects:[],componentInfo:{}}}),this.hideInlineAddForm(),this.renderSelectorGrid(),typeof admin<"u"&&admin.changeCourse&&await admin.changeCourse(r),alert(`✅ "${t}" kursu oluşturuldu!`)}catch(o){console.error(o),alert("Kurs oluşturulurken hata oluştu: "+o.message)}finally{typeof admin<"u"&&admin.hideLoading&&admin.hideLoading()}},renderManagementModal(){const e="course-management-modal";let t=document.getElementById(e);t||(t=document.createElement("div"),t.id=e,t.className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center hidden",t.innerHTML=`
            ...
            <div class="flex items-center justify-between p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded shadow-sm hover:shadow transition group" draggable="true" ondragstart="CourseManager.dragStart(event, ${r})" ondragover="CourseManager.allowDrop(event)" ondrop="CourseManager.drop(event, ${r})">
                <div class="flex items-center gap-4">
                    ...
                    <div class="w-10 h-10 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded text-2xl">${i}</div>
                    <div>
                        <div class="font-bold text-gray-800 dark:text-gray-100">${n}</div>
                        <div class="text-xs text-gray-500 dark:text-gray-400 font-mono">${a}</div>
                    </div>
                </div>
                <div class="flex items-center gap-2">
