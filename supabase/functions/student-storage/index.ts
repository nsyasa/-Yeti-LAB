...
Deno.serve(async (req) => {
    ...
    try {
        ...
        if (!action || !session_token) {
            return new Response(
                JSON.stringify({ error: 'Missing required fields: action, session_token' }),
...
async function handleCreateUpload(
    ...
    if (!ALLOWED_FILE_TYPES.includes(content_type)) {
        return new Response(
            JSON.stringify({ error: 'File type not allowed', allowed: ALLOWED_FILE_TYPES }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
    }
    ...
    if (file_size > MAX_FILE_SIZE) {
        return new Response(
            JSON.stringify({ error: 'File too large', max_size_mb: MAX_FILE_SIZE / (1024 * 1024) }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
    }
    ...
    if (submission.student_id !== student.id) {
        return new Response(
            JSON.stringify({ error: 'Access denied: You do not own this submission' }),
    ...
    if (assignment.classroom_id !== student.classroom_id) {
        return new Response(
            JSON.stringify({ error: 'Access denied: Assignment not in your classroom' }),
            { status: 403, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
    }
    ...
    if (!['published', 'active', 'closed'].includes(assignment.status)) {
        return new Response(
            JSON.stringify({ error: 'Assignment is not accepting submissions' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
    }
...
async function handleDeleteFile(
    ...
    return new Response(
        JSON.stringify({ success: true, deleted_file_id: file_id }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
}
